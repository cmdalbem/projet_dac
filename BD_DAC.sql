...
CREATE TABLE `client` (
    ...
    FOREIGN KEY (`num_carte`)
        REFERENCES `carteFidelite` (`num_carte`)
        ON DELETE CASCADE
);
...
CREATE TABLE `produitAAcheter` (
    `idProduit` INTEGER UNSIGNED NOT NULL PRIMARY KEY,
    FOREIGN KEY (`idProduit`)
        REFERENCES `produitStocke` (`idProduit`)
        ON DELETE CASCADE
);
...
CREATE TABLE `achete` (
    ...
    PRIMARY KEY (idProduit , idClient),
    FOREIGN KEY (`idProduit`)
        REFERENCES `produitAAcheter` (`idProduit`)
        ON DELETE CASCADE,
    FOREIGN KEY (`idClient`)
        REFERENCES `client` (`idClient`)
        ON DELETE CASCADE
